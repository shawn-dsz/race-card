  ...
  render: function() {
    return (
      <div className="r-result">
        ...
        {this.props.oddWin ? <div className="r-result-item">${this.props.oddWin}</div> : null }
        {this.props.oddPlace ? <div className="r-result-item">${this.props.oddPlace}</div> : null }
        {this.props.winningTime ? <div className="r-result-item">{this.props.winningTime}</div> : null }
        {this.props.margin ? <div className="r-result-item">{this.props.margin}</div> : null }
      </div>
    );
  }
});
...
var RaceCardResults = React.createClass({
  render: function() {
    var results = this.props.results.map(function(result){
      return (
        <RaceResult
          ...
          winningTime={result.WinningTime}
    ...
    return (
      <div className="r-card__detail r-card__section">
        {results}
      </div>
    );
  }
...
var RaceCardHeader = React.createClass({
  render: function() {
    return (
      <div className="r-card__header r-card__section">
  ...
  render: function() {
    ...
    return (
      <section className="r-card">
        ...
        />
        <RaceCardActions />
        <RaceCardResults results={this.props.results}/>
      </section>
    );
  }
});
...
var Dashboard = React.createClass({
  ...
  render: function() {
    var hasContent = this.state.raceInfoList.length > 0;
    return (
      <div>
        {hasContent ? <RaceList raceInfoList={this.state.raceInfoList}/> : <div>failed to load.</div>}
      </div>
    );
  }
});
...
$( document ).ready(function() {
  React.render(<Dashboard source="http://localhost:3000/races" />, document.body);
});
